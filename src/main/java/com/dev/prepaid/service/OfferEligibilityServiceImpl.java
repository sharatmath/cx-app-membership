...
//                            validationSubscriberLevel((String) input.get("MSISDN"), eligibleRows, notEligibleRows, invocation.getUuid(), prepaidCxOfferEligibilityList.get());
    ...
    public List<List<String>> evaluationOfferLevelCondition(List<List<String>> rows, InvocationRequest invocation, PrepaidCxOfferConfig instanceConfiguration) throws Exception {
       ...
       //save to db
        if (rows.size() > 0)
            processOfferLevelCap(rows, prepaidCxOfferEligibilityList.get(), invocation.getUuid(), instanceConfiguration, prepaidCxOfferSelectionList.get(0));
        ...
        if (instanceConfiguration != null) {
            if (ProvisionType.DIRECT_PROVISION.getDescription().equals(instanceConfiguration.getProvisionType())) {
                ...
                o.put("msisdn", rows);
                o.put("offerMembershipId", rows);
                o.put("smsKeyword", "");
                sendToRedemptionQueue(o);
            }
        }
...
//        prepaidOfferMembershipExclusRepository.saveAll(memberships);
    ...
    private ResponseEntity<String> sendToRedemptionQueue(Map<String, Object> payload) {
